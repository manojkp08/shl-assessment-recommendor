...
#     try:
#         print("🔄 Fetching main catalog...")
#         catalog_response = requests.get(CATALOG_URL, headers={'User-Agent': 'Mozilla/5.0'}, timeout=10)
#         catalog_response.raise_for_status()
#         catalog_soup = BeautifulSoup(catalog_response.text, 'html.parser')
        
#         # Level 1: Extract names and URLs
#         assessments = []
#         rows = catalog_soup.select("table tr")
#         total_assessments = len(rows) - 1  # Subtract header row
        
#         print(f"🔍 Found {total_assessments} assessments. Starting deep scrape...")
...
#             # Level 2: Scrape individual assessment page for description
#             try:
#                 print(f"📄 Fetching description ({i}/{total_assessments}): {assessment_url}")
...
#                 # IMPROVED: SHL description selector based on the screenshot
#                 description = ""
...
#                 time.sleep(2)  # More respectful delay to avoid being blocked

#             except Exception as e:
#                 print(f"⚠️ Failed to scrape {assessment_url}: {str(e)}")
#                 assessments.append({
#                     "name": link.get_text(strip=True),
#                     "url": assessment_url,
...
#         # Save final data
#         with open("data/shl_assessments.json", "w") as f:
#             json.dump(assessments, f, indent=2)
            
#         print(f"✅ Successfully scraped {len(assessments)}/{total_assessments} assessments")
#         return assessments
